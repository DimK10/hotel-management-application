...
public interface HotelRepository extends PagingAndSortingRepository<Hotel,Long>{
	...
	@Query(value = "select count(r) from rooms r inner join Hotel h on h.id = r.hotel.id where h.owner.id = :id")
    Integer countAllRooms(@Param("id") Long id);

	@Query(value = "select count(r) from rooms r inner join Hotel h on r.hotel.id = h.id inner join orders o on o.room.id = r.id  where h.owner.id = :id and :date between o.checkInDate and o.checkOutDate")
